...
| DRAFTER_BASE_URI                        | No       | ""         | Base URI which relative URIs in input data should be expanded with |
...
Due to the limitations of the sesame SPARQL repository, large append
or delete jobs must split the incoming data into batches to be applied
sequentially to the stardog database. The size of these batches can be
configured with ==DRAFTER_BATCHED_WRITE_SIZE== - note that out-of-memory
