...
Drafter uses integrant for its configuration.  It contains a partial base [[configuration][/drafter/resources/drafter-base-config.edn]] inside its jar, and expects additional configuration to provided as EDN files via its command line.
