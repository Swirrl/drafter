...
(deftest find-existing-user-test
  (let [email "test@example.com"
    ...
    (um/insert-user *user-repo* test-user :publisher)
...
(deftest get-all-users-test
  (let [email-f #(str "user" % "@example.com")
        expected-users (reduce
                        (fn [users [i role]]
                          (let [u (user/create-user (email-f i) role (str i))]
                            (um/insert-user *user-repo* u role)
                            (conj users u)))
                        []
                        (map vector (range 1 10) (cycle user/roles)))]
