...
#+BEGIN_EXAMPLE
DRAFTER_BACKEND               (default SESAME_BACKEND)
#+END_EXAMPLE

The currently there are two supported backends: SESAME_NATIVE and SESAME_STARDOG. Different backends have their own
configuration options. There are also common configuration settings which apply across all backends:
