...
(defn live-query [qstr & {:as kwargs}]
  (build-query "/v1/sparql/live" qstr kwargs))


(defn- draftset-query [draftset qstr & {:keys [union-with-live] :as kwargs}]
  (tc/with-identity test-editor
    (-> (str draftset "/query")
        (build-query qstr kwargs)
        (cond-> union-with-live (assoc-in [:params :union-with-live] "true")))))
